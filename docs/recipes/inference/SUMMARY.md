#

- [Predict for Data Augmentation in ITS](./predict2/inference-its/inference.md)
- [Transfer for Data Augmentation in ITS](./transfer1/inference-its-weather-augmentation/inference.md)
- [Transfer for Warehouse Safety](./transfer1/inference-warehouse-mv/inference.md)
- [Transfer for Gr00t-Mimic](./transfer1/gr00t-mimic/inference.md)
- [Transfer for Drive Simulator](./transfer2_5/inference-carla-sdg-augmentation/inference.md)
- [Transfer for Robotics Navigation Tasks](./transfer1/inference-x-mobility/inference.md)
- [Transfer Multi-Control Video Editing](./transfer2_5/inference-real-augmentation/inference.md)
- [Transfer for Data Augmentation in Biodiversity](./transfer2_5/inference-biotrove-augmentation_w_FiftyOne/inference.md)
