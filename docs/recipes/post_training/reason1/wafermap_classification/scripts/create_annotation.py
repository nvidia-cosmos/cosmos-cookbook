...
DEFECT_TYPES = ['loc', 'edge-loc', 'center', 'edge-ring', 'scratch', 'near-full', 'donut', 'random']
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate JSON annotation file for LLaVA")
    parser.add_argument("-r", "--root-path", default="../WM811K_data/train/", help="Root path to image dataset")
    parser.add_argument("-o", "--output-path", default="./annotation/", help="Output path for JSON file")
...
def generate_qa_entries(image_path, label, templates):
    ...
    wrong_idx = random.choice([i for i in range(len(DEFECT_TYPES)) if i != DEFECT_TO_INDEX.get(label, -1)])
    ...
    entries.append(create_entry(
        ...
        multi["multiple_ans_list"][DEFECT_TO_INDEX.get(label, 0)].replace("[label]", label)
    ))
