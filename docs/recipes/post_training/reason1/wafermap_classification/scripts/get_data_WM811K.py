...
def parse_args():
    ...
    parser.add_argument("-d", "--data-path", default="../WM811K.pkl", help="Path to WM811K pickle file")
    parser.add_argument("-o", "--output-dir", default="./WM811K_data", help="Output directory")
    parser.add_argument("-s", "--image-size", type=int, nargs=2, default=[224, 224], help="Image size (H W)")
    parser.add_argument("-t", "--train-count", type=int, default=100, help="Training images per type")
    parser.add_argument("-e", "--test-count", type=int, default=20, help="Testing images per type")
...
def clean_train_test_label(df):
    ...
    def extract(val):
        if isinstance(val, (list, np.ndarray)):
            ...
            if "training" in items: return "Training"
            if "test" in items: return "Test"
...
def save_wafer_image(wafer_map, save_path, args):
    ...
    resized = resize(wafer_map, tuple(args.image_size), order=0, mode="constant", 
                     cval=0, anti_aliasing=False, preserve_range=True)
...
def process_defect_type(wafer_maps, defect_type, args):
    ...
    for i in range(train_n):
        try:
            path = os.path.join(args.output_dir, "train", defect_type, f"wafermap_{defect_type}_{i}.png")
    ...
    for i in range(test_n):
        try:
            idx = train_n + i
            path = os.path.join(args.output_dir, "test", defect_type, f"wafermap_{defect_type}_{idx}.png")
