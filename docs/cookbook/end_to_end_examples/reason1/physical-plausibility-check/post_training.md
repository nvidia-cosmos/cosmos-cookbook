...
???+ code "Prompt for Scoring Physical Plausibility"
    ...
    ```yaml
    --8<-- "docs/cookbook/end_to_end_examples/reason1/physical-plausibility-check/assets/video_reward.yaml"
    ```
...
???+ code "Training Configuration"
    ...
    ```toml
    --8<-- "docs/cookbook/end_to_end_examples/reason1/physical-plausibility-check/assets/sft_config.toml"
    ```
...
???+ code "Prompt for RL Training"
    ...
    ```yaml
    --8<-- "docs/cookbook/end_to_end_examples/reason1/physical-plausibility-check/assets/video_reward_with_thinking.yaml"
    ```
...
???+ code "RL Training Configuration"
    ...
    ```toml
    --8<-- "docs/cookbook/end_to_end_examples/reason1/physical-plausibility-check/assets/rl_config.toml"
    ```
...
**Notes**: The RL training script uses the `custom_grpo.py`, which is modified from the [GRPO script](https://github.com/nvidia-cosmos/cosmos-reason1/blob/main/examples/post_training/tools/dataset/cosmos_grpo.py) in the Cosmos Reason1 repository. We included the implementation of our reward function in `custom_grpo.py`.
