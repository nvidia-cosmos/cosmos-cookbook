...
class Inference:
    ...
    def generate(self, samples: list[InferenceArguments], output_dir: Path) -> list[str]:
        ...
        num_generations = self.setup_args.num_generations
        for i_sample, sample in enumerate(samples):
            log.info(f"[{i_sample + 1}/{len(samples)}] Processing sample {sample.name}")
            for generation_idx in range(num_generations):
                current_seed = sample.seed + generation_idx
                log.info(
                    f"Generating video {generation_idx + 1}/{num_generations} with seed {current_seed}"
                )
                output_path = self._generate_sample(
                    sample, output_dir, generation_idx=generation_idx
                )
    ...
    def _generate_sample(
        ...
        if num_generations <= 1:
            ...
            output_path = output_dir / f"{sample.name}_seed{current_seed}"

        if self.rank0:
            output_dir.mkdir(parents=True, exist_ok=True)
            # Save args and run text guardrail only on first generation per sample
            if generation_idx == 0:
                ...
                if self.text_guardrail_runner is not None:
                    if not guardrail_presets.run_text_guardrail(
                        sample.prompt, self.text_guardrail_runner
                    ):
