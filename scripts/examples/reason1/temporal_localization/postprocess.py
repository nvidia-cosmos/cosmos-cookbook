...
def extract_two_floats(text):
    """
    Extract two float numbers from a string in format like "<1.73> <2.00>".

    Args:
        text (str): Input string containing patterns like "<X.XX> <Y.YY>"

    Returns:
        tuple: Two float numbers found, or (None, None) if not found
    """
    # Pattern to match float numbers inside angle brackets
...
gt_timestamps_toaster = {
    "cut_682545_head_color_fixed": [1.83, 6.43, 9.83, 11.16],
    "cut_681882_head_color_fixed": [1.5, 5.5, 9.83, 11.16],
    "cut_688144_head_color_fixed": [1.66, 7.83, 11.0, 11.9],
    "cut_683236_head_color_fixed": [2.83, 7.46, 10.66, 11.66],
    "cut_688068_head_color_fixed": [2.0, 8.0, 11.06, 11.8],
    "cut_682655_head_color_fixed": [1.66, 6.1, 9.6, 10.43],
    "cut_683270_head_color_fixed": [2.13, 5.96, 9.53, 10.26],
    "cut_687781_head_color_fixed": [1.86, 7.43, 11.66, 14.23],
    "cut_683347_head_color_fixed": [2.23, 7.73, 12.06, 12.73],
    "cut_686921_head_color_fixed": [2.26, 7.3, 11.2, 12.0],
}

gt_timestamps_chips = {
    "cut_724566_head_color_fixed": [3.66, 13.0],
    "cut_725207_head_color_fixed": [2.5, 9.66],
    "cut_725261_head_color_fixed": [2.16, 10.5],
    "cut_723523_head_color_fixed": [2.33, 11.66],
    "cut_724955_head_color_fixed": [4.0, 14.53],
    "cut_725248_head_color_fixed": [2.33, 10.0],
    "cut_723493_head_color_fixed": [2.46, 8.83],
    "cut_725170_head_color_fixed": [2.5, 9.83],
    "cut_724153_head_color_fixed": [2.66, 11.16],
    "cut_723270_head_color_fixed": [4.76, 13.83],
}

gt_timestamps_fork = {
    "cut_686443_head_color_fixed": [4.76, 9.0, 11.16, 14.0],
    "cut_686066_head_color_fixed": [4.2, 9.6, 11.5, 14.0],
    "cut_686488_head_color_fixed": [3.33, 9.0, 11.46, 14.26],
    "cut_685938_head_color_fixed": [2.0, 7.66, 10.66, 14.66],
    "cut_686045_head_color_fixed": [5.66, 9.33, 11.16, 13.33],
    "cut_686094_head_color_fixed": [3.06, 7.16, 10.5, 13.93],
    "cut_686180_head_color_fixed": [3.33, 8.0, 10.53, 14.16],
    "cut_686320_head_color_fixed": [2.16, 6.93, 10.63, 14.5],
    "cut_686245_head_color_fixed": [5.0, 9.0, 10.93, 14.0],
    "cut_686470_head_color_fixed": [2.3, 7.6, 10.6, 14.33],
}

gt_timestamps_cup = {
    "cut_709524_head_color_fixed": [4.33, 8.16, 13.5],
    "cut_709556_head_color_fixed": [2.33, 7.5, 14.43],
    "cut_709507_head_color_fixed": [3.66, 7.16, 14.16],
    "cut_709325_head_color_fixed": [2.66, 6.46, 14.0],
    "cut_709342_head_color_fixed": [3.33, 6.76, 13.83],
    "cut_710323_head_color_fixed": [2.0, 6.66, 14.33],
    "cut_710150_head_color_fixed": [3.2, 8.23, 14.33],
    "cut_709286_head_color_fixed": [3.33, 7.0, 14.33],
    "cut_709987_head_color_fixed": [3.0, 7.6, 14.33],
    "cut_709401_head_color_fixed": [2.16, 6.16, 14.33],
}
...
Examples:
  ...
  python postprocess.py /path/to/results --gt-timestamps cube_stacking --fps 4
    ...
    parser.add_argument(
        "results_dir", type=str, help="Directory containing the results pickle files"
    )
    ...
    # Print configuration
    print(
        f"Using {'END' if use_end_time else 'START'} times ({'odd' if use_end_time else 'even'} indices)"
    )
    print(f"FPS: {fps}")
    ...
    for demo_name in ["0"]:
        ...
        results_file = f"{results_dir}/results_demo{demo_name}.pkl"
        ...
        for i in range(num_events):
            if i == 0:
                subtask_durations.append(
                    gt_timestamps[demo_name][0]
                )  # Duration until first event
            else:
                subtask_durations.append(
                    gt_timestamps[demo_name][i] - gt_timestamps[demo_name][i - 1]
        ...
        for trial in range(num_trials):
            ...
            if not output_text_fps:
                print(
                    f"Demo {demo_name} fps {fps} trial {trial}: No output text, skipping..."
                )
                continue
            # Check if trial index is within bounds
            if trial >= len(output_text_fps):
                print(
                    f"Trial {trial}: Index out of range (only {len(output_text_fps)} trials available), skipping..."
                )
                continue
            ...
            # Extract predictions from output - only from <answer> block
            tmp_list = []

            # Extract text between <answer> and </answer> tags
            answer_match = re.search(
                r"<answer>(.*?)</answer>", output_text, re.DOTALL | re.IGNORECASE
            )
            if answer_match:
                answer_text = answer_match.group(1)
            else:
                # If no answer tags, use the whole output text as fallback
                answer_text = output_text

            # Extract event lines and their timestamps from answer block only
            event_lines = [i for i in answer_text.split("\n") if "Event" in i]
            extracted_values = [list(extract_two_floats(i)) for i in event_lines]

            if extracted_values:
                flattened = np.ravel(extracted_values)
            ...
            tmp_list = [
                i
                for i in tmp_list
                if len(i) == expected_length and all(x is not None for x in i)
            ]

            if len(tmp_list) == 0:
                print(f"Trial {trial}: No valid predictions extracted, skipping...")
                print(f"  Found {len(event_lines)} event lines in answer block")
                if extracted_values:
            ...
            for i in tmp_list:
                ...
                for event_idx in range(num_events):
                    ...
                    if use_end_time:
                        pred_end_time = i[
                            event_idx * 2 + 1
                        ]  # End time is at odd indices
                    else:
                        pred_end_time = i[
                            event_idx * 2
                        ]  # Start time is at even indices
            ...
            trial_errors_percent = [
                (trial_errors_seconds[i] / subtask_durations[i] * 100)
                if subtask_durations[i] > 0
                else 0
                for i in range(num_events)
            ]
        ...
        if len(demo_trials_errors_seconds) > 0:
            ...
            print(
                f"Average errors (seconds): {demo_avg_errors_seconds} ± {demo_std_errors_seconds}"
            )
            print(
                f"Average errors (percent): {demo_avg_errors_percent} ± {demo_std_errors_percent}"
            )

            # Print event-specific metrics dynamically
            for event_idx in range(num_events):
                print(
                    f"Event {event_idx + 1} - Avg error: {demo_avg_errors_seconds[event_idx]:.3f}s ± {demo_std_errors_seconds[event_idx]:.3f}s ({demo_avg_errors_percent[event_idx]:.1f}% of {subtask_durations[event_idx]:.2f}s subtask)"
                )

            print(
                f"Overall avg error: {np.mean(demo_avg_errors_seconds):.3f}s ({np.mean(demo_avg_errors_percent):.1f}%)"
            )

            # Print hit rates dynamically
            hit_rate_str = ", ".join(
                [f"Event{i+1}={demo_avg_hit_rates[i]:.3f}" for i in range(num_events)]
            )
            ...
            if len(demo_trials_predictions) > 0:
                ...
                for event_idx in range(num_events):
                    column_names.extend(
                        [f"event{event_idx+1}_start", f"event{event_idx+1}_end"]
                    )

                predictions_df = pd.DataFrame(
                    demo_trials_predictions, columns=column_names
                )
                predictions_df["trial"] = range(len(demo_trials_predictions))
                predictions_df.to_csv(
                    f"{output_dir}/results_stat_demo_{demo_name}_fps{fps}_all_trials.csv",
                    index=False,
                )
            ...
            for event_idx in range(num_events):
                trials_error_dict[f"event{event_idx+1}_error_s"] = [
                    e[event_idx] for e in demo_trials_errors_seconds
                ]
                trials_error_dict[f"event{event_idx+1}_error_pct"] = [
                    e[event_idx] for e in demo_trials_errors_percent
                ]
                trials_error_dict[f"event{event_idx+1}_hit_rate"] = [
                    h[event_idx] for h in demo_trials_hit_rates
                ]

            trials_error_df = pd.DataFrame(trials_error_dict)
            trials_error_df.to_csv(
                f"{output_dir}/per_trial_stats_demo_{demo_name}_fps{fps}.csv",
                index=False,
            )
            ...
            demo_summary_df.to_csv(
                f"{output_dir}/summary_demo_{demo_name}_fps{fps}.csv", index=False
            )
    ...
    if len(all_demos_errors_seconds) > 0:
        ...
        max_events = (
            mean_errors_seconds.shape[0]
            if len(mean_errors_seconds.shape) > 0
            else len(mean_errors_seconds)
        )

        # Print subtask durations
        duration_str = ", ".join(
            [f"{mean_subtask_durations[i]:.2f}s" for i in range(max_events)]
        )
        ...
        for event_idx in range(max_events):
            print(
                f"Event {event_idx + 1} - Mean error: {mean_errors_seconds[event_idx]:.3f}s ± {std_errors_seconds[event_idx]:.3f}s ({mean_errors_percent[event_idx]:.1f}% ± {std_errors_percent[event_idx]:.1f}%)"
            )
        ...
        print(
            f"Overall mean error: {np.mean(mean_errors_percent):.1f}% ± {np.mean(std_errors_percent):.1f}%"
        )

        # Print hit rates dynamically
        hit_rate_str = ", ".join(
            [
                f"Event{i+1}={mean_hit_rates[i]:.3f}±{std_hit_rates[i]:.3f}"
        ...
        aggregate_df.to_csv(
            f"{output_dir}/aggregate_stats_all_demos_fps{fps}.csv", index=False
        )

        # Save detailed per-demo comparison (averaged across trials for each demo)
        demo_keys = [
            k
            for k, v in zip(gt_timestamps.keys(), all_demos_errors_seconds)
            if v is not None
        ]
        ...
        # Dynamically add columns for each event
        for event_idx in range(max_events):
            comparison_dict[f"event{event_idx+1}_error_s"] = all_demos_errors_seconds[
                :, event_idx
            ]
            comparison_dict[f"event{event_idx+1}_error_pct"] = all_demos_errors_percent[
                :, event_idx
            ]
            comparison_dict[f"event{event_idx+1}_hit_rate"] = all_demos_hit_rates[
                :, event_idx
            ]

        comparison_df = pd.DataFrame(comparison_dict)
        comparison_df.to_csv(
            f"{output_dir}/per_demo_comparison_fps{fps}.csv", index=False
        )
        ...
        print(
            f"Per-demo comparison saved to: {output_dir}/per_demo_comparison_fps{fps}.csv"
        )
        print("=" * 70)
    else:
