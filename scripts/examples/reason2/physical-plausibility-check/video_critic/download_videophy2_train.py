...
def download_video(
    video_url: str, video_index: int, dataset_name: str, split: str
) -> Optional[str]:
...
def main():
    ...
    if args.first_n is not None:
        if args.first_n > len(dataset):
            print(
                f"⚠️ Warning: --first_n ({args.first_n}) is larger than dataset size ({len(dataset)}). Using all samples."
            )
        else:
    ...
    def process_sample(sample_with_index: tuple) -> Optional[dict]:
        ...
        local_video_path = download_video(
            video_url, video_index + 1, args.dataset, args.split
        )
