...
"""Convert cosmos-curate webdataset to huggingface dataset."""
...
def _replace_column_name(url: str, old_name: str, new_name: str) -> str:
    if url.count(f"/{old_name}/") != 1:
        raise ValueError(f"Name {old_name} not found in {url}")
    return url.replace(f"/{old_name}/", f"/{new_name}/")


def _get_shard_urls(dataset_path: str, *, column_name: str = "metas") -> list[str]:
...
def _join_webdatasets(datasets: list[Iterable], column_names: list[str]) -> Iterable:
...
def _load_webdataset(
    ...
    urls_0 = _get_shard_urls(dataset_path, column_name=col_name_0)
    datasets = []
    for col_name in column_names:
        urls = [_replace_column_name(url, col_name_0, col_name) for url in urls_0]
        datasets.append(webdataset.WebDataset(urls, **dataset_kwargs))
    return _join_webdatasets(datasets, column_names)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("input", type=str, help="Input webdataset directory.")
    parser.add_argument(
    ...
    # Load
    dataset = _load_webdataset(args.input, ["metas", "video"])
