...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> list[dict]:
        ...
        try:
            ...
            raise ValueError(f"Invalid conversation data at index {idx}: {e}") from e
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise FileNotFoundError(f"Cannot read config file {args.config}: {e}") from e
    except toml.TomlDecodeError as e:
        raise ValueError(f"Invalid TOML format in {args.config}: {e}") from e
    ...
    try:
        hf_dataset = datasets.load_from_disk(custom_config.dataset.path)
        ...
        raise RuntimeError(f"Failed to load or validate dataset: {e}") from e
