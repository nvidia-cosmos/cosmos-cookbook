...
def load_dataset(path: str) -> datasets.Dataset:
    # add a __key__ in data
    data = {"video": [], "metas": [], "__key__": []}

    # Get base names from video folder
    video_dir = os.path.join(path, "videos")
    text_dir = os.path.join(path, "metas")
    # t5_dir = os.path.join(path, "t5_xxl")
    for video_file in os.listdir(video_dir):
        if video_file.endswith(".mp4"):
            base = os.path.splitext(video_file)[0]

            txt_file = os.path.join(text_dir, base + ".txt")
            # pkl_file = os.path.join(t5_dir, base + ".pickle")

            if os.path.exists(txt_file):
                # Add video path
                data["video"].append(os.path.join(video_dir, video_file))

                # Add text
                with open(txt_file, "r") as f:
                    data["metas"].append(f.read().strip())

                # Add T5-XXL pickle
                #with open(pkl_file, "rb") as f:
                #    t5_features = pickle.load(f)
                #    data["t5_xxl"].append(t5_features)
                data["__key__"].append(base)
...
def main():
    ...
    parser.add_argument(
        "--prompt", type=str, default="./cosmos-curate/configs/prompt_0.yaml", help="Path to prompt yaml file"
    )
    parser.add_argument("--vision-config", type=str, default="./cosmos-curate/configs/vision_config.json", help="Path to vision config json file")
    parser.add_argument("--generation-config", type=str, default="./cosmos-curate/configs/generation_config.json", help="Path to generation config json file")
    parser.add_argument(
        "--model",
        type=str,
        default="nvidia/Cosmos-Reason1.1-7B",
